...
async function parseAudioMetadata(blob, _metadataCallback, _errorCallback) {
	return new Promise((res, err) => {
		function errorCallback(...args) {
			if (_errorCallback) _errorCallback(...args);
			err(...args);
		}
		function metadataCallback(...args) {
			if (_metadataCallback) _metadataCallback(...args);
			res(...args);
		}
		...
		function parseID3v2Metadata(header) {
			...
			function readPic(view, size, id) {
				...
				return blob.slice(picstart, picstart + piclength, mimetype);
			}
		...
		function parseOggMetadata(header) {
			...
			header.getMore(p2_offset, p2_length, function (page, error) {
				...
				for (var i = 0; i < num_comments; i++) {
					if (page.remaining() < 4) {
						// 4 bytes for comment-length variable
		...
		function parseMP4Metadata(header) {
			...
			function parseMoovAtom(data, end) {
				...
				while (data.index < end) {
					...
					if (type === "udta") {
						// Metadata is inside here
						parseUdtaAtom(data, end);
						data.index = nextindex;
					} else if (type === "trak") {
						// We find the audio format inside here
		...
		getUTF8Text: function (offset, len) {
			...
			while (pos < end) {
				...
				if (b1 < 128) {
					...
					charcode = ((b1 & 0x07) << 18) + ((b2 & 0x3f) << 12) + ((b3 & 0x3f) << 6) + (b4 & 0x3f);
